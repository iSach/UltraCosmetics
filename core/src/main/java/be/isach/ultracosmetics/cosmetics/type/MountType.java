...
public class MountType extends CosmeticEntType<Mount> {
    ...
    private MountType(String configName, XMaterial material, EntityType entityType, String defaultDescription, int repeatDelay, List<XMaterial> defaultBlocks, double defaultSpeed, Class<? extends Mount> mountClass) {
        super(Category.MOUNTS, configName, defaultDescription, material, entityType, mountClass);
    ...
    public static void register() {
        new MountType("DruggedHorse", XMaterial.SUGAR, EntityType.HORSE, "&7&oThat is just too much!", 2, null, 1.1, MountDruggedHorse.class);
        new MountType("GlacialSteed", XMaterial.PACKED_ICE, EntityType.HORSE, "&7&oThis mount comes from North Pole!", 2, Arrays.asList(XMaterial.SNOW_BLOCK), 0.4, MountGlacialSteed.class);
        new MountType("MountOfFire", XMaterial.BLAZE_POWDER, EntityType.HORSE, "&7&oThe mount of Hades!", 2, Arrays.asList(XMaterial.ORANGE_TERRACOTTA, XMaterial.YELLOW_TERRACOTTA, XMaterial.RED_TERRACOTTA), 0.4, MountOfFire.class);
        new MountType("Snake", XMaterial.WHEAT_SEEDS, EntityType.SHEEP, "&7&oWatch out! It may bite..", 2, null, 0.3, MountSnake.class);
        new MountType("Dragon", XMaterial.DRAGON_EGG, EntityType.ENDER_DRAGON, "&7&oBecome a dragon rider!", 1, null, 0.7, MountDragon.class);
        new MountType("Slime", XMaterial.SLIME_BALL, EntityType.SLIME, "&7&oSplat! Splat!", 2, null, 0.8, UltraCosmeticsData.get().getVersionManager().getModule().getSlimeClass());
        new MountType("HypeCart", XMaterial.MINECART, EntityType.MINECART, "&7&oEver wanted to drive a F1?\n&7&oNow you can!", 1, null, 0, MountHypeCart.class);
        new MountType("Spider", XMaterial.COBWEB, EntityType.SPIDER, "&7&oYOU are the spider jockey!", 2, null, 0.4, UltraCosmeticsData.get().getVersionManager().getModule().getSpiderClass());
        new MountType("MoltenSnake", XMaterial.MAGMA_CREAM, EntityType.MAGMA_CUBE, "&7&oDeep under the Earth's surface, there\n&7&oexists a mythical species of Molten\n&7&oSnakes. This one will serve you eternally.", 1, null, 0.4, MountMoltenSnake.class);
